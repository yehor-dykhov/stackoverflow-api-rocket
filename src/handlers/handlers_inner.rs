...
pub async fn create_question(
    ...
    match question {
        ...
        Err(err) => {
            error!("Couldn't to create the question: {}", err);
...
pub async fn read_questions(
    ...
    match questions {
        ...
        Err(err) => {
            error!("Couldn't to get questions: {}", err);
            Err(HandlerError::default_internal_error())
        }
    }
}
...
pub async fn create_answer(
    ...
    match answer {
        Ok(answer) => Ok(answer),
        Err(err) => {
            error!("Couldn't to create the answer: {}", err);

            match err {
                DBError::InvalidUUID(ref s) => Err(HandlerError::BadRequest(s.to_string())),
